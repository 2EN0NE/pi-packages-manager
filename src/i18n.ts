...
const SEED_TRANSLATIONS: Record<string, Record<string, string>> = {
	"zh-CN": {
		// 用户已安装的包
		"pi-tinyfish-tools":
			"TinyFish 网页代理工具 — 搜索、抓取和目标驱动的浏览器自动化",
		"pi-autoname": "AI 驱动的会话命名 — 通过 LLM 生成有意义的语义化会话名称",
		"pi-i18n":
			"多语言支持 — 让 pi 能说中文、日本語、한국어、Español 等多种语言",
		"@juicesharp/rpiv-ask-user-question":
			"结构化问卷扩展 — 当模型无法猜测时，向用户提供带选项的问卷",
		"@juicesharp/rpiv-todo":
			"待办事项列表 — 以实时叠加层渲染，支持 /reload 和对话压缩后恢复",
		"@juicesharp/rpiv-i18n":
			"rpiv 系列插件的本地化基础 — 语言检测、/languages 命令、跨包语言注册表",
		"@gerdloos/npm-trusts-github-skill":
			"npm 可信发布技能 — 教 LLM 如何通过 GitHub Actions 配置 OIDC 发布，无需 token",
		// 热门社区包
		"@samfp/pi-memory":
			"持久化记忆 — 从会话中学习修正、偏好和模式，并注入到未来的对话中",
		...
		"@cryptolibertus/pi-peer":
			"本地 pi 对等消息 — 本地 pi 实例之间的消息传递、命令、工具和运行时传输",
	...
	ja: {
		"pi-tinyfish-tools":
			"TinyFish Webエージェントツール — 検索、フェッチ、ゴール駆動ブラウザ自動化",
		"pi-autoname":
			"AI駆動セッション命名 — LLMでセマンティックなセッション名を生成",
		"pi-i18n":
			"多言語サポート — piで日本語、中文、한국어、Españolなどを話せるように",
		"@juicesharp/rpiv-ask-user-question":
			"構造化アンケート拡張 — モデルが推測できない場合にユーザーに選択肢付きの質問を提示",
		"@juicesharp/rpiv-todo":
			"ToDoリスト — リアルタイムオーバーレイで表示、/reloadと圧縮後に復元",
		"@juicesharp/rpiv-i18n":
			"rpivシリーズのi18n基盤 — 言語検出、/languagesコマンド、クロスパッケージ言語レジストリ",
		"@gerdloos/npm-trusts-github-skill":
			"npmトラストパブリッシングスキル — GitHub ActionsでOIDCパブリッシュを設定する方法をLLMに教える",
	},
	ko: {
		"pi-tinyfish-tools":
			"TinyFish 웹 에이전트 도구 — 검색, 페치, 목표 기반 브라우저 자동화",
		"pi-autoname": "AI 기반 세션 이름 지정 — LLM으로 의미 있는 세션 이름 생성",
		"pi-i18n":
			"다국어 지원 — pi에서 한국어, 日本語, 中文, Español 등을 사용 가능",
		"@juicesharp/rpiv-ask-user-question":
			"구조화된 질문 확장 — 모델이 추측할 수 없을 때 사용자에게 선택지가 있는 질문 제공",
		"@juicesharp/rpiv-todo":
			"할 일 목록 — 실시간 오버레이로 렌더링, /reload 및 압축 후 복원",
		"@juicesharp/rpiv-i18n":
			"rpiv 시리즈의 i18n 기반 — 언어 감지, /languages 명령, 크로스 패키지 언어 레지스트리",
		"@gerdloos/npm-trusts-github-skill":
			"npm 트러스트 퍼블리싱 스킬 — GitHub Actions로 OIDC 퍼블리시를 설정하는 방법을 LLM에 가르침",
	},
};
...
export function getTranslatedDescriptions(
	...
	for (const pkg of packages) {
		result.set(
			pkg.name,
			getTranslatedDescription(pkg.name, pkg.description, locale),
		);
	}
	return result;
}
...
export function localizeType(type: string, locale?: Locale): string {
	const loc =
		(locale || detectLocale()).split("-")[0] === "zh"
			? (locale || detectLocale()).startsWith("zh-TW")
				? "zh-TW"
				: "zh-CN"
...
const UI_TEXT: Record<string, Record<string, string>> = {
	...
	"zh-CN": {
		...
		"settings.preferences.reset": "[p] 重置所有偏好到默认",
		...
		"settings.tip.config":
			"提示：在终端运行 `pi config` 可启用 / 禁用扩展、技能、提示词、主题。",
		"settings.locale.changed": "语言已切换为",
		// translation settings
		"settings.section.translation": "本地翻译",
		"settings.translation.url": "MTranServer 地址",
		"settings.translation.apikey": "API Key",
		"settings.translation.status": "服务状态",
		"settings.translation.status.ok": "✅ 已连接",
		"settings.translation.status.err": "❌ 未连接",
		"settings.translation.status.unknown": "◐ 未检测",
		"settings.translation.cache": "翻译缓存",
		"settings.translation.cached": "个包缓存",
		"settings.translation.cache.empty": "空",
		"settings.translation.shortcuts":
			"[m] 修改地址  [k] 修改 Key  [y] 测试连接  [x] 清空缓存",
	},
	// ── 繁體中文 ──
	"zh-TW": {
		"command.description":
			"🔌 插件管理器：檢視、搜尋、安裝、解除安裝 pi 擴展插件",
		...
		"settings.preferences.reset": "[p] 重設所有偏好為預設",
		...
		"settings.tip.config":
			"提示：在終端執行 `pi config` 可啟用 / 停用擴充、技能、提示詞、主題。",
		"settings.locale.changed": "語言已切換為",
		"settings.section.translation": "本地翻譯",
		"settings.translation.url": "MTranServer",
		"settings.translation.apikey": "API Key",
		"settings.translation.status": "狀態",
		"settings.translation.status.ok": "✅ 已連接",
		"settings.translation.status.err": "❌ 未連接",
		"settings.translation.status.unknown": "◐ 未檢測",
		"settings.translation.cache": "翻譯快取",
		"settings.translation.cached": "個套件快取",
		"settings.translation.cache.empty": "空",
		"settings.translation.shortcuts":
			"[m] 修改地址  [k] 修改 Key  [y] 測試  [x] 清空快取",
	},
	// ── English ──
	en: {
		"command.description":
			"🔌 Plugin Manager: view, search, install, and uninstall pi extensions",
		...
		"settings.preferences.reset": "[p] Reset all preferences to defaults",
		"settings.confirm.reset":
			"This will clear your language preference and restore defaults. Continue?",
		...
		"settings.tip.config":
			"Tip: run `pi config` in your terminal to enable/disable extensions, skills, prompts, and themes.",
		"settings.locale.changed": "Language switched to",
		"settings.section.translation": "Local Translation",
		"settings.translation.url": "MTranServer URL",
		"settings.translation.apikey": "API Key",
		"settings.translation.status": "Service",
		"settings.translation.status.ok": "✅ Connected",
		"settings.translation.status.err": "❌ Disconnected",
		"settings.translation.status.unknown": "◐ Untested",
		"settings.translation.cache": "Translation cache",
		"settings.translation.cached": "packages cached",
		"settings.translation.cache.empty": "Empty",
		"settings.translation.shortcuts":
			"[m] URL  [k] Key  [y] Test  [x] Clear cache",
	},
	// ── 日本語 ──
	ja: {
		"command.description":
			"🔌 プラグインマネージャー：pi拡張プラグインの表示・検索・インストール・アンインストール",
		...
		"ai.placeholder":
			"必要な機能を説明してください。例：UI関連のプラグインが欲しい",
		...
		"settings.preferences.reset":
			"[p] すべてのプリファレンスをデフォルトに戻す",
		"settings.confirm.reset":
			"言語プリファレンスをクリアし、デフォルトに戻します。続行しますか？",
		"settings.refreshed":
			"カタログキャッシュを更新しました（{count} パッケージ）",
		...
		"settings.tip.config":
			"ヒント: ターミナルで `pi config` を実行して拡張・スキル・プロンプト・テーマを制御します。",
		"settings.locale.changed": "言語を切り替えました:",
		"settings.section.translation": "ローカル翻訳",
		"settings.translation.url": "MTranServer URL",
		"settings.translation.apikey": "API Key",
		"settings.translation.status": "サービス状態",
		"settings.translation.status.ok": "✅ 接続済み",
		"settings.translation.status.err": "❌ 未接続",
		"settings.translation.status.unknown": "◐ 未テスト",
		"settings.translation.cache": "翻訳キャッシュ",
		"settings.translation.cached": "パッケージキャッシュ",
		"settings.translation.cache.empty": "空",
		"settings.translation.shortcuts":
			"[m] URL  [k] Key  [y] テスト  [x] クリア",
	},
	// ── 한국어 ──
	ko: {
		"command.description":
			"🔌 플러그인 관리자: pi 확장 플러그인 보기, 검색, 설치, 제거",
		...
		"ai.title": "🧠 AI 시맨틱 검색",
		"ai.placeholder":
			"원하는 기능을 설명하세요. 예: UI 관련 플러그인이 필요해요",
		...
		"settings.preferences.reset": "[p] 모든 환경설정 기본값으로 초기화",
		"settings.confirm.reset":
			"언어 환경설정을 지우고 기본값으로 되돌립니다. 계속하시겠습니까?",
		...
		"settings.tip.config":
			"팁: 터미널에서 `pi config`를 실행하여 확장/스킬/프롬프트/테마를 활성화/비활성화하세요.",
		"settings.locale.changed": "언어가 다음으로 전환되었습니다:",
		"settings.section.translation": "로컬 번역",
		"settings.translation.url": "MTranServer URL",
		"settings.translation.apikey": "API Key",
		"settings.translation.status": "서비스 상태",
		"settings.translation.status.ok": "✅ 연결됨",
		"settings.translation.status.err": "❌ 연결 안 됨",
		"settings.translation.status.unknown": "◐ 미확인",
		"settings.translation.cache": "번역 캐시",
		"settings.translation.cached": "개 패키지 캐시",
		"settings.translation.cache.empty": "비어 있음",
		"settings.translation.shortcuts":
			"[m] URL  [k] Key  [y] 테스트  [x] 비우기",
	},
};
