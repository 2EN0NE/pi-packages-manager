...
const CACHE_FILE = join(
	HOME,
	".pi/agent/cache/pi-packages-manager/catalog.json",
);
...
function getInstalledPackagesCached(): PackageInfo[] {
	...
	if (
		...
		now - installedPackagesCacheTime < INSTALLED_CACHE_TTL_MS
	) {
...
export function clearCatalogCache(): void {
	...
	try {
		writeFileSync(
			CACHE_FILE,
			JSON.stringify({ fetchedAt: 0, packages: [] }, null, 2),
			"utf-8",
		);
	} catch {
...
export function getCatalogCacheInfo(): {
	...
	try {
		...
		const fetchedAt =
			typeof parsed.fetchedAt === "number" ? new Date(parsed.fetchedAt) : null;
 ...
 */
export async function refreshCatalogCache(): Promise<{
	success: boolean;
	info: ReturnType<typeof getCatalogCacheInfo>;
}> {
	try {
		catalogCache = null;
...
/**
 * 将运行时 catalog 写入磁盘缓存，供下次会话使用。
 * 面板搜索按 Enter 合并 npm 结果后调用此函数持久化。
 */
export function persistCatalogCache(packages: PackageInfo[]): void {
	catalogCache = packages;
	writeCatalogCache(packages);
}
...
export async function fetchFullCatalog(
	...
	const ranked = rankPackages(allResults, "", allResults.length);
...
export async function searchNpmRegistry(
	...
	// 无搜索词时（仅 filter 如 type:extension），不需要查 npm registry
	if (!registryQuery) {
		const catalog = await fetchFullCatalog(Math.max(size, 250));
		return rankPackages(catalog, query, size);
	}
...
export async function getPackageDetail(
	...
	try {
		...
		const latestVersion = data.versions?.[latest || ""] as
		...
		return {
			...
			types: extractTypesFromManifest(
				pkgJson.pi as Record<string, unknown> | undefined,
			),
		...
		needsCheck.map(async (pkg) => {
			...
			if (!response.ok) {
				return {
					...
					hasUpdate: false,
					skipReason: `registry ${response.status}`,
				} as UpdateInfo;
			}
			const data = (await response.json()) as Record<string, unknown>;
			const latest = (data["dist-tags"] as Record<string, string>)?.latest;
			const hasUpdate = Boolean(
				latest && pkg.installedVersion && latest !== pkg.installedVersion,
			);
	...
	for (let i = 0; i < checkResults.length; i++) {
		...
		if (result.status === "fulfilled") {
			...
			results.push({
				...failedPkg,
				hasUpdate: false,
				skipReason: (result.reason as Error)?.message || "check failed",
			});
		}
	}
	...
	for (const line of lines) {
		...
		for (const pkg of catalog) {
			...
			if (
				cleaned === pkg.name ||
				cleaned === shortName ||
				cleaned.includes(pkg.name) ||
				pkg.name.includes(cleaned)
			) {
				seen.add(pkg.name);
				matched.push(pkg);
				break;
 ...
 * 不会阻塞事件循环。
 */
export async function detectNpmCachePermissionIssuesAsync(): Promise<{
	count: number;
	samplePaths: string[];
}> {
 ...
 */
export function detectNpmCachePermissionIssues(): {
	count: number;
	samplePaths: string[];
} {
	try {
...
async function walkDirForBadOwner(
	dir: string,
	results: string[],
	limit: number,
	depth: number,
 ...
 * 失败时返回可操作的修复建议。
 */
export async function fixNpmCachePermissionsAsync(): Promise<{
	fixed: boolean;
	message: string;
}> {
 ...
 */
export async function ensureNpmCacheHealthyAsync(): Promise<{
	ok: boolean;
	message: string;
}> {
...
export function runPiInstall(pkgName: string): {
	success: boolean;
	output: string;
} {
	...
	if (quickCheck.count > 0) {
		return {
			success: false,
			output: `npm cache permission error: run 'sudo chown -R $(whoami) ~/.npm' in your terminal.`,
		};
	}
	try {
...
/** 异步版安装，不会冻结 UI 事件循环 */
export async function runPiInstallAsync(
	pkgName: string,
	...
	if (quickCheck.count > 0) {
		...
		if (!cacheCheck.ok) {
			return {
				success: false,
				output: `npm cache permission error:\n${cacheCheck.message}`,
			};
		}
	}
	...
	const source = normalizeInstallSource(pkgName);
	const args =
		scope === "project" ? ["install", source, "-l"] : ["install", source];
...
export async function runPiInstallStreaming(
	...
	return new Promise((resolve) => {
		...
		child.stdout?.on("data", (chunk: Buffer | string) =>
			emitLines(String(chunk), false),
		);
		child.stderr?.on("data", (chunk: Buffer | string) =>
			emitLines(String(chunk), true),
		);
		...
		const finalize = (code: number | null, signal: NodeJS.Signals | null) => {
			...
			if (code === 0 && !signal) {
				...
				resolve({
					success: false,
					output: enhanceNpmError(combined || `Install failed (exit ${code})`),
				});
			}
		};
...
export function runPiUninstall(pkgName: string): {
	success: boolean;
	output: string;
} {
	// Quick sync pre-flight cache check
...
/** 异步版卸载，不会冻结 UI 事件循环 */
export async function runPiUninstallAsync(
	pkgName: string,
	scope?: "user" | "project",
): Promise<{ success: boolean; output: string }> {
	// Pre-flight: check npm cache health (lightweight sync check first, async deep check only if needed)
	...
	const source = normalizeInstallSource(pkgName);
	const args =
		scope === "project" ? ["uninstall", source, "-l"] : ["uninstall", source];
	return new Promise((resolve) => {
		execFile(
			"pi",
			args,
			...
			(err, stdout, stderr) => {
				if (err) {
					const rawOutput =
						stderr || stdout || err.message || "Uninstall failed";
					const output = enhanceNpmError(rawOutput);
					resolve({ success: false, output });
				} else {
					resolve({ success: true, output: stdout });
...
export function removeFromSettings(pkgName: string): boolean {
	...
	for (const settingsFile of [SETTINGS_FILE, PROJECT_SETTINGS_FILE]) {
		...
		const filtered = packages.filter(
			(ref) => ref !== targetRef && !(npmName && ref === `npm:${npmName}`),
		);
 ...
 */
export async function fetchNpmDownloadsBulk(
	names: string[],
...
async function rawNpmSearch(
	...
	return data.objects.map((obj) =>
		mapNpmObject(obj, installedPkgs, installedNames),
	);
}
...
function writeCatalogCache(packages: PackageInfo[]): void {
	try {
		...
		writeFileSync(
			CACHE_FILE,
			JSON.stringify({ fetchedAt: Date.now(), packages }, null, 2),
			"utf-8",
		);
	} catch {
...
function mergeInstalledState(packages: PackageInfo[]): PackageInfo[] {
	...
	return packages.map((pkg) => {
		...
		return {
			...
			piManifest:
				Object.keys(installed.piManifest || {}).length > 0
					? installed.piManifest
					: pkg.piManifest,
...
function enhanceNpmError(rawOutput: string): string {
	const isPermissionError =
		rawOutput.includes("EACCES") || rawOutput.includes("permission denied");
	const isEexistError =
		rawOutput.includes("EEXIST") || rawOutput.includes("File exists");
...
function readPackageRefs(): PackageRef[] {
	...
	for (const [settingsFile, scope] of [
		[SETTINGS_FILE, "user"],
...
function readLocalPackage(ref: string): PackageInfo | null {
	...
	const pkgJsonPath = packagePath.endsWith("package.json")
		? packagePath
...
function createFallbackPackage(
	...
	return {
		name,
		description:
			ref.startsWith("git:") || ref.startsWith("http") || ref.startsWith("ssh:")
...
function inferTypesFromKeywords(keywords: string[]): string[] {
	...
	const types: string[] = [];
	if (lower.includes("pi-extension") || lower.includes("extension"))
		types.push("extension");
	if (lower.includes("pi-skill") || lower.includes("skill"))
		types.push("skill");
	if (lower.includes("pi-prompt") || lower.includes("prompt"))
		types.push("prompt");
	if (lower.includes("pi-theme") || lower.includes("theme"))
		types.push("theme");
...
export function normalizeInstallSource(input: string): string {
	...
	if (
		/^(npm:|git:|https?:\/\/|ssh:\/\/|file:)/.test(value) ||
		value.startsWith("./") ||
		value.startsWith("../") ||
		value.startsWith("/")
	) {
