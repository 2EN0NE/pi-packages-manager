...
export default function pluginManager(pi: ExtensionAPI) {
	...
	pi.registerCommand("packages-list", {
		...
		getArgumentCompletions: (prefix: string) => {
			const subcommands = [
				...
				{
					value: "settings",
					label: "settings: view installed package sources",
				},
		...
		handler: async (args, ctx) => {
			...
			switch (sub) {
				...
				case "search":
					if (rest) {
						// 有查询词时：直接输出文本结果
						await showSearchResults(rest, ctx);
					} else {
						// 无查询词时：进入交互式浏览
	...
	async function panelLoop(ctx: ExtensionCommandContext) {
		...
		while (true) {
			...
			if (result.action === "change-locale") {
				...
				const labelEntry = SUPPORTED_LOCALES.find(
					(entry) => entry.code === result.locale,
				);
			...
			if (result.action === "settings-refresh-catalog") {
				...
				if (refreshResult.success) {
					ctx.ui.notify(
						t("settings.refreshed", locale, {
							count: refreshResult.info.count,
						}),
						"info",
					);
				} else {
	...
	async function showMainMenu(ctx: ExtensionCommandContext): Promise<boolean> {
		...
		if (choice.includes(t("menu.installed", locale))) {
			...
			choice.includes(t("browse.title", locale)) ||
			choice.includes("Community") ||
			choice.includes("社区") ||
			choice.includes("社群") ||
			choice.includes("コミュニティ") ||
			choice.includes("커뮤니티")
		) {
			await browseCommunity(ctx, "");
	...
	async function listInstalled(ctx: ExtensionCommandContext) {
		...
		installed.forEach((pkg, i) => {
			if (compactView) {
				...
				const desc = getTranslatedDescription(
					pkg.name,
					pkg.description,
					locale,
				);
				...
				items.push(
					`${circleNum(i + 1)} ${pkg.name}${version}${formatMeta(pkg)}  · ${descShort}`,
				);
			} else {
				const desc = getTranslatedDescription(
					pkg.name,
				...
				const typeLabels =
					pkg.types?.map((tp) => localizeType(tp, locale)).join(", ") || "";
				const version = pkg.installedVersion
					? `  v${pkg.installedVersion}`
					: "";
				if (i > 0) items.push(""); // 空行分隔
				items.push(
					`${circleNum(i + 1)} ${pkg.name}${version}${typeLabels ? `  [${typeLabels}]` : ""}${formatMeta(pkg)}\n    ${desc}`,
				);
			}
		});
	...
	async function browseCommunity(
		...
		showLoading(
			...
			query
				...
				: t("browse.loading", locale),
		);

		let items: PackageInfo[];
		try {
			items = query
				? await searchNpmRegistry(query, 60)
				: await fetchFullCatalog();
		} catch (err) {
			clearLoading(ctx);
			ctx.ui.notify(
				`${t("search.error", locale)}: ${(err as Error).message}`,
				"error",
			);
			return;
		}
		...
		if (items.length === 0) {
			ctx.ui.notify(
				query ? t("search.no_results", locale) : t("browse.empty", locale),
				"info",
			);
			return;
		}
		...
		const pageTitle = query
			? `${t("browse.title", locale)} · "${query}"`
			: t("browse.title", locale);
		await paginatedView(items, pageTitle, ctx);
	}

	/**
	 * 文本输出模式：搜索 pi 包并直接返回结果列表（非交互）。
	 * 用于 `/packages-list search <query>` 且带查询词时。
	 */
	async function showSearchResults(
		...
		let items: PackageInfo[];
		try {
			items = await searchNpmRegistry(query, 60);
		} catch (err) {
			clearLoading(ctx);
			ctx.ui.notify(
				`${t("search.error", locale)}: ${(err as Error).message}`,
		...
		if (items.length === 0) {
			ctx.ui.notify(t("search.no_results", locale), "info");
			return;
		}

		// 输出结果
		const lines = items.map((pkg, i) => {
			...
			return `${i + 1}. ${pkg.name}${badge}${dl}${types} — ${desc}`;
		});

		ctx.ui.notify(
			[`📦 ${items.length} package(s) matching "${query}":`, "", ...lines].join(
				"\n",
			),
			"info",
		);
		...
		while (true) {
			...
			pageItems.forEach((pkg, i) => {
				if (compactView) {
					...
					const descShort = desc.length > 60 ? desc.slice(0, 57) + "…" : desc;
					pluginOpts.push(
						`${circleNum(start + i + 1)} ${pkg.name}${badge}${dl}${meta}  · ${descShort}`,
					);
				} else {
					// 详细模式：多行
					...
					const reasons = pkg.searchReasons?.length
						...
						: "";
					pluginOpts.push(
						`${circleNum(start + i + 1)} ${pkg.name}${badge}${dl}${meta}\n    ${desc}${reasons}`,
					);
				}
			});
			...
			const labelWidth = [...t("nav.section", locale)].reduce(
				(w, c) => w + (c.charCodeAt(0) > 0x7f ? 2 : 1),
				0,
			);
			...
			// 跳过提示栏、分隔线、操作标签
			if (selIdx <= 1 || selected.startsWith("─") || selected.startsWith("├"))
				continue;
			...
			if (action) {
				...
				if (tag === "search") {
					...
					try {
						...
						const sub = await paginatedView(
							r,
							`${t("browse.title", locale)} · "${q}"`,
							ctx,
						);
				...
				if (tag === "ai") {
					...
					try {
						...
						clearLoading(ctx);
						if (r.length === 0) {
							ctx.ui.notify(t("ai.no_results", locale), "info");
							continue;
						}
						const sub = await paginatedView(
							r,
						...
						clearLoading(ctx);
						ctx.ui.notify(
							`${t("ai.error", locale)}: ${(err as Error).message}`,
							"error",
						);
					}
				}
		...
		while (true) {
			...
			const info = detail
				? {
						...pkg,
						...detail,
						downloads: detail.downloads ?? pkg.downloads,
						searchScore: pkg.searchScore,
						searchReasons: pkg.searchReasons,
					}
				: pkg;
			...
			if (hasUpdate) {
				lines.push(
					`⬆️  ${t("detail.update", locale)}: ${info.installedVersion} → ${info.latestVersion}`,
				);
			}

			if (info.source) lines.push(`source: ${info.source}`);
			if (info.sourceType || info.scope)
				lines.push(
					`scope: ${[info.scope, info.sourceType].filter(Boolean).join(" · ")}`,
				);
			if (info.searchReasons?.length)
				lines.push(`match: ${info.searchReasons.join(", ")}`);
			if (info.version)
				lines.push(`${t("detail.version", locale)}: ${info.version}`);
			if (info.latestVersion && !info.installed)
				lines.push(`${t("detail.latest", locale)}: ${info.latestVersion}`);
			if (info.author)
				lines.push(`${t("detail.author", locale)}: ${info.author}`);
			if (info.license)
				lines.push(`${t("detail.license", locale)}: ${info.license}`);
			if (info.types?.length)
				lines.push(
					`${t("detail.types", locale)}: ${info.types.map((tp) => localizeType(tp, locale)).join(", ")}`,
				);
			...
			if (info.downloads)
				lines.push(
					`${t("detail.downloads", locale)}: ${formatNumber(info.downloads)}/mo`,
				);
			if (info.keywords?.length)
				lines.push(
					`${t("detail.keywords", locale)}: ${info.keywords.join(", ")}`,
				);
			...
			if (
				choice.includes("Run security audit") ||
				choice.includes("Re-run audit")
			) {
	...
	async function installPackageFlow(
		pkgName: string,
		ctx: ExtensionCommandContext,
	) {
		if (!pkgName) {
			pkgName =
				(await ctx.ui.input(
					t("detail.install", locale),
					"package name (e.g. pi-tinyfish-tools)",
				)) || "";
		}
		...
		const result = await runPiInstallStreaming(pkgName, scope, (line) => {
		...
		if (result.success) {
			...
			ctx.ui.notify(
				...
				audit.overallRisk === "high" || audit.overallRisk === "critical"
					? "warning"
					: "info",
			);
			clearCatalogCache();
			...
			ctx.ui.notify(
				`❌ ${t("install.fail", locale)}: ${result.output}`,
				"error",
			);
		}
	}
	...
	async function removePackageFlow(
		pkgName: string,
		ctx: ExtensionCommandContext,
	): Promise<boolean> {
		if (!pkgName) {
			pkgName =
				(await ctx.ui.input(t("detail.remove", locale), "package name")) || "";
		}
		...
		const command =
			selectedRef.scope === "project"
				? `pi uninstall ${uninstallSource} -l`
		...
		const result = await runPiUninstallAsync(
			uninstallSource,
			selectedRef.scope,
		);
		...
		if (result.success) {
			clearLoading(ctx);
			ctx.ui.notify(
				`✅ ${pkgName} ${t("remove.success", locale)}\nRun /reload or restart Pi to unload removed resources.`,
				"info",
			);
			clearCatalogCache();
			return true;
		...
		if (removed) {
			ctx.ui.notify(
				`✅ ${pkgName} removed from settings. ${t("remove.success", locale)}`,
				"info",
			);
			clearCatalogCache();
			return true;
			...
			ctx.ui.notify(
				`❌ ${t("remove.fail", locale)}: Package not found in settings.`,
				"error",
			);
			return false;
		}
	...
	async function updatePackages(pkgName: string, ctx: ExtensionCommandContext) {
		...
		const skipped = updates.filter(
			(p) => !p.hasUpdate && (p.pinned || p.skipReason),
		);

		if (withUpdates.length === 0) {
			const skippedLines = skipped.map(
				(p) => `• ${p.name} — ${p.skipReason || "skipped"}`,
			);
		...
		withUpdates.forEach((pkg) => {
			...
			const meta = formatMeta(pkg);
			items.push(
				`${pkg.name}: ${pkg.installedVersion} → ${pkg.latestVersion}${meta}`,
			);
		});
		...
		if (skipped.length) {
			...
			skipped.forEach((pkg) => {
				items.push("");
				items.push(
					`• ${pkg.name}${formatMeta(pkg)} — ${pkg.skipReason || "skipped"}`,
				);
			});
		}
		...
		const target = withUpdates.find((pkg) =>
			selected.startsWith(`${pkg.name}: `),
		);
		if (!target) return;
		...
		if (result.success) {
			ctx.ui.notify(
				`✅ ${target.name} ${t("update.success", locale)}\nRun /reload or restart Pi to activate updated resources.`,
				"info",
			);
			clearCatalogCache();
		} else {
		...
		while (true) {
			...
			const pushGroup = (title: string, group: typeof refs) => {
				...
				group.forEach((ref) => {
					...
					items.push("");
					items.push(
						`• ${ref.ref}${pinned}${version}${types}  — ${sourceType}`,
					);
			...
			if (pkg) {
				...
				if (proceed) {
					...
					ctx.ui.notify(
						removed
							? `✅ Removed ${matchedEntry.ref.ref} from settings.`
							: `❌ Could not remove ${matchedEntry.ref.ref}.`,
						removed ? "info" : "error",
					);
				}
			}
	...
	async function showPackageInfo(
		...
		if (!pkgName) {
			pkgName =
				(await ctx.ui.input(t("detail.version", locale), "package name")) || "";
		}
		if (!pkgName.trim()) return;
...
async function chooseRemovalTarget(
	...
	const fallback = {
		ref: normalizeInstallSource(pkgName),
		scope: "user" as const,
	};
	if (refs.length === 0) {
		const proceed = await ctx.ui.confirm(
...
function formatSecuritySummary(pkg: PackageInfo): string[] {
	...
	else if (pkg.sourceType === "git")
		lines.push("  Source: git/remote repository");
	...
	lines.push(
		"  Note: Pi packages may run arbitrary code. Review trusted sources before installing.",
	);
	return lines;
}
