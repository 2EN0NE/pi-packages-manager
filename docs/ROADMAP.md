...
| # | 特性 | 说明 | 工作量 |
|---|------|------|--------|
| **C** | 🌐 pi.dev 文字元数据富化 | 动态检测可用 web fetch 工具（tinyfish / web-fetch 等），搜索结果补全 pi.dev 上的分类标签、简短描述等文字元数据（不做截图——pi 是 CLI/TUI 工具，终端里渲染不了图片） | 1 天 |
...
v1.3.0 ✅ Settings 扩展（缓存状态 + 快捷操作 + 偏好重置 + 语言来源）
  │
  ▼
v1.3.2 ✅ 版本号同步 pi.dev ← 当前
  │
  ▼
v1.3.x 🔜 质量 + 扩展（pi.dev 文字元数据富化 + 扩展测试）
  │
  ▼
